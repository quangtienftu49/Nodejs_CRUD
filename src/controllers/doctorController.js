...
let getTopDoctorHome = async (req, res) => {
  ...
  try {
    let response = await doctorService.getTopDoctorHome(+limit); //+ to convert string to integer
...
let getPatientListForDoctor = async (req, res) => {
  try {
    let infor = await doctorService.getPatientListForDoctor(
      req.query.doctorId,
      req.query.date
    );
    return res.status(200).json(infor);
